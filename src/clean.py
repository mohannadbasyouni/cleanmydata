...
def clean_data(
    ...
    *,
    ...
    from src.utils import Spinner, write_log
    import time
    ...
    try:
        ...
        summary.update(
            {"rows": df.shape[0], "columns": df.shape[1], "duration": duration}
        )
...
def remove_duplicates(
    ...
    if verbose:
        if removed > 0:
            _vprint(f"• Duplicate rows removed: {removed} ({pct:.1f}%)", color="cyan", verbose=verbose)
            _vprint(
...
def normalize_column_names(df, verbose=False):
    ...
    if verbose:
        ...
        if renamed_pairs:
            ...
            max_len = min(max(len(o) for o, _ in renamed_pairs) + 2, shutil.get_terminal_size((80, 20)).columns // 2)
...
def clean_text_columns(df, lowercase=True, verbose=False, categorical_mapping=None):
    ...
    if verbose:
        ...
        _vprint("  - Stripped whitespace, standardized spacing, normalized casing.", color="dim", verbose=verbose)
...
def normalize_categorical_text(df, mapping=None, verbose=False):
    ...
    for col, col_map in mapping.items():
        ...
        if verbose:
            ...
            _vprint(f"• Normalized '{col}' ({len(mapped_keys)} mapped)", color="cyan", verbose=verbose)
            if len(col_map) <= 5:
...
def standardize_formats(df, verbose=False):
    ...
    if verbose:
        if converted_dt or converted_num:
            ...
            _vprint(f"• {total_converted} column(s) standardized successfully", color="green", verbose=verbose)
            _vprint(f"• {total_text - total_converted} text columns unconverted", color="green", verbose=verbose)
            _vprint("──────────────────────────────", color="dim", verbose=verbose)
...
def handle_outliers(df, method="cap", auto_detect=True, verbose=False):
    ...
    for col in numeric_cols:
        ...
        _vprint(f"• {col:<25} → {outlier_count} outliers handled "
                f"[{method_used}, skew={skew:.2f} ({skew_desc})]",
                color="cyan", verbose=verbose)
        _vprint(f"  - low={low_outliers}, high={high_outliers}",
                color="dim", verbose=verbose)
    ...
    if verbose:
        if method == "remove" and outliers_removed > 0:
            ...
            _vprint(f"• Outliers capped across {len(outliers_capped)} column(s) "
                    f"({total_capped} total).", color="cyan", verbose=verbose)
        else:
            _vprint("• No significant outliers detected.", color="cyan", verbose=verbose)

        # ---------- SAMPLE PREVIEW ----------
        if method == "remove" and not removed_samples.empty:
            _vprint(f"  Example of removed outlier rows (first {len(removed_samples)} "
                    f"of {removed_total} shown):", color="dim", verbose=verbose)
            table = Table(show_header=True, header_style="bold white", box=box.MINIMAL)
...
def fill_missing_values(df, verbose=False, numeric_strategy='auto', datetime_strategy='median'):
    ...
    for col in df.columns:
        ...
        if series.dropna().empty:
            ...
            _vprint(f"• {col:<25} → {missing_count} missing filled [{method_used}]", color="cyan", verbose=verbose)
            continue
        ...
        # ---------- BOOLEAN ----------
            ...
            df[col] = series.fillna(fill_val)
            _vprint(f"• {col:<25} → {missing_count} missing filled [mode (bool)]", color="cyan", verbose=verbose)
        ...
        # ---------- CATEGORY ----------
            ...
            df[col] = series.fillna(fill_val)
            _vprint(f"• {col:<25} → {missing_count} missing filled [mode (category)]", color="cyan", verbose=verbose)
        ...
        # ---------- OBJECT / STRING ----------
            ...
            fill_val = mode_val[0] if not mode_val.empty else "Unknown"
            df[col] = series.fillna(fill_val)
            _vprint(f"• {col:<25} → {missing_count} missing filled [mode (string)]", color="cyan", verbose=verbose)
